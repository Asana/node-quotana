...
App.prototype._startUi = function() {
  ...
  me._uiShowing = true;
  me.ui().html(
      '<div class="quotana-quote-container"><div class="quotana-quote"></div></div>');
  me.reset();
  me.cycleQuotes();
};
...
App.prototype.start = function() {
  ...
  // Create controls to change quote filters
  me._renderControls();
...
App.prototype._renderControls = function() {
  var me = this;

  function appendControl(source) {
    $('#source_controls').append(
            '<a class="control-link" id="control_view_' + source +
            '" href="#">' + source + '</a>');
    $('#control_view_' + source).click(function() {
      me.parameters.source = source;
  ...
  var sourcesSeen = {};
  me.options.projects.forEach(function(project) {
    if (!(project.source in sourcesSeen)) {
      sourcesSeen[project.source] = true;
      appendControl(project.source);
    }
  });

  // Built-in filter for all sources
...
App.prototype.reset = function() {
  ...
  // Set css class on outermost container so appearance can be totally
  // customized per source.
  $('#quotana').attr(
      'class',
      'source-' + (me.parameters.source ? me.parameters.source : 'all'));
};
...
App.prototype.swapQuote = function(newQuote) {
  ...
  var quoteContainer = $('.quotana-quote-container');
  ...
  quoteContainer.fadeOut(1000, function() {
    ...
    quoteContainer.get(0).className = '';
    quoteContainer.addClass('quotana-quote-container');
    quoteContainer.addClass('quotana-type-' + newQuote.type);
    quoteContainer.addClass('quotana-source-' + newQuote.source);
    quoteContainer.css({
      left: ($(window).width() - quoteContainer.outerWidth()) / 2,
      top: ($(window).height() - quoteContainer.outerHeight()) / 2
    }).fadeIn(1000);
  });
